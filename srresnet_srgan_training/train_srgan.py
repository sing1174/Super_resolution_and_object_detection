...
def main():
    ...
    print(f"Train images: {len(train_loader)*batch_size}" )

    # Total number of epochs to train for
    epochs = int(iterations // len(train_loader) + 1)

    #epochs = min(80,int(iterations // len(train_loader) + 1))
    # Epochs
    print(f"Total epochs: {epochs}")
    
    # Initialize wandb
    wandb.init(
        project="SRGAN-training",  # Replace with your project name
        config={  # Track hyperparameters
            "learning_rate": lr,
            "batch_size": batch_size,
            "scaling_factor": scaling_factor,
            "n_blocks_g": n_blocks_g,
            "n_blocks_d": n_blocks_d,
            "beta": beta,
        }
    )
    ...
    for epoch in range(start_epoch, epochs):
        ...
        #wandb.save(f'checkpoint_srgan_epoch_{epoch}.pth.tar')
...
def train(train_loader, generator, discriminator, truncated_vgg19, content_loss_criterion, adversarial_loss_criterion,
    ...
    for i, (lr_imgs, hr_imgs) in enumerate(train_loader):
        ...
        wandb.log({
            "content_loss": losses_c.avg,
            "adversarial_loss": losses_a.avg,
            "discriminator_loss": losses_d.avg,
